...
Plots.histogram(agg_daily_wind, nbins = 100, title = "Daily generation potential of reanalysis data - Wind", legend = false)
    
allwind = hcat([sum(my_sims[i].wind, dims=2) for i in 1:48]...)
Plots.histogram(vec(allwind), nbins = 100,  title = "Daily generation potential of simulated data - Wind", legend = false)


# probability density function - solar
agg_daily_solar = sum(input.solar, dims = 2)
Plots.histogram(agg_daily_solar, nbins = 100, title = "Daily generation potential of reanalysis data - Solar", legend = false)
    
allsolar = hcat([sum(my_sims[i].solar, dims=2) for i in 1:48]...)
Plots.histogram(vec(allsolar), nbins = 100, title =  "Daily generation potential of simulated data - Solar", legend = false)
